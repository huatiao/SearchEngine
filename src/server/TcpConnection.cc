...
void TcpConnection::sendJson(const json &msg)
{
    string buf = msg.dump();
    _sockIO.sendMsg(buf.c_str());
}

void TcpConnection::sendInLoop(const json &msg)
{
    if(_loop)
    {
        _loop->runInLoop(std::bind(&TcpConnection::sendJson, this, msg));//注册到EventLoop
...
json TcpConnection::recvJson()
{
    char buff[65535] = {0};
    _sockIO.recvMsg(buff);
